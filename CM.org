...
Taux de miss [définition]

Taux de miss usuel pour les instructions : 2 à 5 %.
Taux de miss usuel pour les données : 5 à 10 %.

Pour 1/1000 de rapport entre le cache et la mémoire.


Les autres types de caches : set associative

*** L'associatif par ensemble de blocs

Si on fait l'hypothèse qu'on a un cache à 8 emplacements, on créé 4 ensembles de deux emplacements chacun.

Chaque bloc peut atterrir non plus dans un emplacement, mais dans n'importe lequel des deux emplacements du même ensemble.

La partition optimale est celle qui éloigne le plus les deux colonnes (à reprendre en français). La distance minimale entre deux 

Une adresse pour le cache sera donc maintenant formatée comme suit :

- Toujours le offset depuis le début du bloc (écrit sur log_2(nombre d'octets du bloc) bits)
- Un numéro d'ensemble et non plus un numéro d'emplacement (écrit sur log_2(nombre d'ensembles du cache) bits) (un peu moins long)
- Tag (les bits de poids fort) (un peu plus long maintenant)
...
le but final : on veut se débarasser du bloc tel que pour une suite arbitraire d'opérations du processeur, le taux de cache miss soit le plus bas possible.

La stratégie FIFO : on vire le plus ancien.
La stratégie LIFO : on vire le plus nouveau.
La stratégie LRU : on vire celui qui a été accédé pour la dernière fois il y a plus longtemps.

La dernière stratégie est basée sur la localité temporelle. (le direct mapping ne permet pas de se servir de cette propriété)

[Illustration du principe de localité temporelle, petite distribution]

On a besoin d'un seul bit pour stocker lequel des deux blocs de l'ensemble a 

Puisque on peut avoir deux blocs par ensemble, le matériel change.

On a besoin de deux mémoires directory par ensemble, deux mémoires data.

On a besoin de vérifier, grâce au numéro de tag, lequel des deux blocs nous intéresse.

[photo du matériel]

Un peu plus lent à l'accès.

set associative est le cas général, direct mapping en est un cas particulier.

bits LRU : décrire l'algorithme : complexité de 2^n n le nombre de bits LRU.

Algo simplifié à toujours 1 bit. [décrire]

Tradeoff.

*** Full associative

Cas particulier, où on a un seul ensemble avec tous les emplacements dedans.
...
Miss de capacité : plusieurs blocs se battent pour l'ensemble des emplacements (plus de place)
...
Les problèmes de cohérence mémoire.

Plusieurs processeurs, chacun avec son petit cache.

Notion de cohérence :

Cohérence en séquentialité : Si un processeur Pi écrit B à l'adresse @0, puis il lit l'adresse @0, il doit obtenir B.

Cohérence en validité itérative : Si un processeur Pi écrit A à l'adresse @0, si le processeur Pj lit itérativement @0, il doit obtenir A au bout d'un certain temps fini.

Cohérence en ordonnancement itérative : Si Pi écrit A à l'adresse @0, puis il écrit B à l'adresse @0, si Pj lit itérativement @0, il ne peut jamais lire B puis A.
...
Cohérence est respectée, et principe simple.
Inconvénient est que chaque écriture requiert d'accéder au bus.

Deuxième stratégie possible :
Write back : On oublie la mémoire primaire, on se contente de modifier la valeur dans le cache. Et on sauvegarde les modifications dans la mémoire centrale au moment où on s'apprête à supprimer.

Accès en écriture sont très rapide.
Mais la cohérence mémoire n'est plus assurée. (marche probablement quand même quand on a un seul processeur).
...
Dans le cas du cache WT : si on a choisi WT, c'est que les écritures sont distribuées : pas le coup d'amener le bloc.
Dans le cas du cache WB : si on a choisi WB, c'est que les écritures sont localisées : vaut le coup de ramener le bloc.
...
Cette quantité de 1024 pas forcément fixée.
Plus il y a de niveaux d'indirection, plus on peut se permettre d'avoir une quantité faible, ce qui permettra de bien compacter l'espace que prendra la table des pages.
...
La bonne manière, c'est la table de hachage.
(on va en plus se permettre de cacher les correspondances)
...
*(ça suppose que j'ai toujours l'adresse physique de la table des pages)*
Il y a donc un registre qui stocke cette adresse physique constante.
