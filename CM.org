...
Chaque registre peut contenir 32 bits, soit un entiers.
...
\forall i \in \lbracket 0 ; 2^n - 1 \rbracket, \exists \alpha un vecteur binaire (dont tous les ) tq :
...
On peut shift la range des nombres écrivables de $0 ; 2^n - 1$ à $-2^{n-1} ; 2^{n-1} - 1$ en changeant le vecteur à :
