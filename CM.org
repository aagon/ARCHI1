...
Considérons une suite d'instructions processeur arbitraire, et en particulier le sous-ensemble qui font des accès mémoire. Toutes les instructions font un accès mémoire, dans le Instruction Fetch. Un sous-ensemble d'entre-elles font en plus un autre accès mémoire (en lecture ou en écriture) dans l'étage mem.

On a donc deux types d'accès mémoire bien distincts, les accès instructions (un par instruction) et les accès données (0 ou 1 par instruction).

Le taux de miss est la proportion de ces instructions qui occasionnent un miss.
#+END_DEFINITION

Pour les instructions, le taux de miss couramment retenu est autour de 2 à 5%.
Pour les données, le taux de miss couramment retenu est autour de 5 à 10%.
...
La fonction de densité de probabilité de (X_t = x) conditionnée à (X_{t-1} = x_0) sera une espèce de pointe légèrement dissymétrique à droite (pourquoi ? Exercice pour le lecteur...) centrée autour de x_0.

La fonction de densité de probabilité de (X_t = x) conditionnée à (X_{t-10} = x_0) sera une espèce de montagne légèrement dissymétrique à droite (pourquoi ? Exercice pour le lecteur...) centrée autour de x_0.

La fonction de densité de probabilité de (X_t = x) conditionnée à (X_{t-100} = x_0) sera une espèce de colline légèrement dissymétrique à droite (pourquoi ? Exercice pour le lecteur...) centrée autour de x_0.

La fonction de densité de probabilité de (X_t = x) conditionnée à (X_{t-1000000} = x_0) sera probablement indistinguable de la probabilité a priori.
...
#+BEGIN_DEFINITION
La notion de cohérence se base sur trois critères :

Cohérence en séquentialité : Si un processeur P_i écrit B à l'adresse @0, puis il lit l'adresse @0, il doit obtenir B.

Cohérence en validité itérative : Si un processeur P_i écrit A à l'adresse @0, si le processeur P_j lit itérativement @0, il doit obtenir A au bout d'un certain temps fini.

Cohérence en ordonnancement itérative : Si P_i écrit A à l'adresse @0, puis il écrit B à l'adresse @0, si Pj lit itérativement @0, il ne peut jamais lire B puis A.
#+END_DEFINITION
